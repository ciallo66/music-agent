      ...
      <li v-for="(item, index) in card.items" :key="item.id" class="track-row">
        <router-link :to="`/songs/${item.id}`" class="track-link">
          ...
          <div class="track-meta-right">
            <span class="track-score" v-if="item.match_score"
        ...
        <div class="track-actions">
          ...
          <el-button size="small" type="primary" @click="onAddToCollection(item.id)"
            >加入集合</el-button
          >
          <el-button size="small" @click="onPreview(item.id)">立即预览</el-button>
        </div>
      </li>
    </ul>
...
const feedbackActions = computed(() =>
  props.actions.map((a) => ({
    ...a,
    icon: feedbackIcon(a.action),
  })),
)

function feedbackIcon(action: string): string {
...
function onPreview(songId: number) {
  emit('openAgent', { type: 'preview', query: `播放预览歌曲ID ${songId}` })
}
</script>
